# AGENTS.md

Guía para agentes o asistentes que consuman estos apuntes DDS.

## Objetivo

Ayudar a estudiar Desarrollo de Software usando el apunte principal como fuente primaria.

## Orden de lectura recomendado

1. `README.md`
2. `Apuntes_Desarrollo_de_Software.md`
3. `docs/indice-clases.md`
4. `docs/metodologia-apuntes.md`
5. `study/` para práctica y material derivado

## Reglas

- No inventar fuentes que no estén en el repositorio.
- Priorizar `Apuntes_Desarrollo_de_Software.md` para explicar conceptos.
- Usar `docs/indice-clases.md` para ubicar estado y contexto de cada clase.
- Si una clase figura como `draft`, `parcial` o `pendiente`, aclararlo al responder.
- Mantener español rioplatense natural, tono pedagógico y directo.
- Para estudiar, explicar problema, solución, ejemplo y gotcha; no hacer resúmenes planos.
- Si el apunte referencia una fuente externa no presente, tratarla como referencia histórica/contextual, no como archivo disponible.
